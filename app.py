...
def index():
    ...
    get_featured_posts = \
        mongo.db.gear.aggregate([{'$match': {'is_featured': True}},
                                {'$sample': {'size': 3}}])
    gear_sorted = gear.find().sort('datecreated', -1).limit(6)
    return render_template('index.html',
                           rdm_feat=list(get_featured_posts),
                           gear_collection=list(gear_sorted),
...
def delete_account(user_id):
    ...
    if 'username' in session:
        if session['username'] \
                == users.find_one({'_id': ObjectId(user_id)})['name']:
            del_acc = users.find_one({'_id': ObjectId(user_id),
...
def get_gear():
    ...
    return render_template('gallery.html', gear_collection=gear.find(),
                           categories=list(categories.find()))
...
def gear_details(gear_id):
    ...
    return render_template('gear_details.html',
...
def browsecategory(category):
    ...
    return render_template('browsecategory.html', h2_title=h2_title,
                           post_by_category=list(category_posts),
                           categories=list(categories.find()))
...
def edit_gear(gear_id):
    ...
    return render_template('edit_gear.html', gear=the_gear,
                           category_sel=all_categories,
                           categories=list(categories.find()))
...
def update_gear(gear_id):
    ...
    date_created = \
        gear.find_one({'_id': ObjectId(gear_id)})['datecreated']
    is_featured = \
        gear.find_one({'_id': ObjectId(gear_id)})['is_featured']
